...
class Oauth(private val auth: dev.castive.jmp.api.Auth): EndpointGroup {
    ...
    override fun addEndpoints() {
        ...
        get("${Runner.BASE}/v2/oauth/refresh", { ctx ->
            ...
            val newToken = TokenProvider.get().create(user.username, user.id.value.toString()) ?: throw BadRequestResponse()
            val newRequestToken = TokenProvider.get().create(user.username) ?: throw BadRequestResponse()
            transaction {
                user.requestToken = newRequestToken
            }
